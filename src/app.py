...
#　推論した合計金額を返す関数
#def getName(output):
#    logger.info(f"c_app.py : getName {output}")
#    return output
...
def predicts():
    ...
    if request.method == 'POST':
        # ファイルがなかった場合の処理
        logger.info(f"c_app.py : filename = {'filename'}")
        ...
        if file and allowed_file(file.filename):
            ...
            parser.add_argument("--path", default='./datafolder/saved_image.jpg', help="path to images or video")
            parser.add_argument("--save_result", action="store_true", help="whether to save the inference result of image/video")

            args = make_parser().parse_args()
            logger.info(f"c_app.py : config_path = {config_path}, agrs.demo = {args.demo}")
            logger.info(f"c_app.py : config_path = {config_path}, agrs.save_result C = {args.save_result}")
            exp = get_exp(config_path, args.demo)

            coinTotal_, save_file_name = main(exp, args)
            logger.info(f"c_app.py : save_file_name = {save_file_name}")
